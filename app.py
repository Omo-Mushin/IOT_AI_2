...
# #     #     'Discharge Press psi': np.random.normal(3830, 5, len(dates)),  # Added discharge pressure
...
# #                  'Disc Press (psi)', 'Motor Temp (F)']].dropna()
...
# #     features = df[['Freq (Hz)', 'Current (Amps)', 'Intake Press psi', 'Disc Press (psi)']].dropna()
...
# #     features = df[['Freq (Hz)', 'Current (Amps)', 'Intake Press psi', 'Disc Press (psi)', 'Motor Temp (F)']].dropna()
...
# #     # Prepare data
# #     X = df[['Freq (Hz)', 'Current (Amps)', 'Intake Press psi', 'Disc Press (psi)', 'Motor Temp (F)']].dropna()
...
# #     X = df[['Freq (Hz)', 'Current (Amps)', 'Intake Press psi', 'Disc Press (psi)']].dropna()
...
# #     press_diff = last_reading.get('Discharge Press psi', 3830) - last_reading.get('Intake Press psi', 3140)
...
# #         - **Discharge Pressure**: Pump output pressure (3830±10 psi)
...
# #         esp_data['Pressure Diff'] = esp_data['Disc Press (psi)'] - esp_data['Intake Press psi']
# #         fig = px.line(
# #             esp_data, x='DateTime', y='Pressure Diff',
# #             title='Pressure Differential (Discharge - Intake)',
# #             template='plotly_white'
# #         )
# #         fig.add_hline(y=689, line_dash="dash", line_color="green",
# #                      annotation_text="Normal Range: 680-720 psi", 
# #                      annotation_position="bottom right")
# #         st.plotly_chart(fig, use_container_width=True, key="press_diff_line")
...
# #                     - Pressure Diff: {(row.get('Disc Press (psi)', 3830) - row.get('Intake Press psi', 3140)):.0f} psi
# #                     """)
...
# #                 <li>Normal operating range (35Hz, 18.5A)</li>
...
#     # Replace invalid entries and convert to numeric
#     to_numeric_cols = ['Freq (Hz)', 'Current (Amps)', 'Intake Press psi', 
#                       'Disc Press psi', 'Motor Temp (F)']  # Updated to Disc Press
#     df.replace('-', np.nan, inplace=True)
#     for col in to_numeric_cols:
#         df[col] = pd.to_numeric(df[col], errors='coerce')
#     return df
...
#     return report  # Removed the feature importance plot return
...
# esp_data, kmeans_model, cluster_profiles = cluster_equipment_states(esp_data)
# feature_importance, rca_model = analyze_anomaly_causes(esp_data)
# report_text = generate_insight_reports(esp_data, temp_model)  # Updated
...
# with col4:
#     # Pressure differential
#     press_diff = last_reading.get('Disc Press psi', 3830) - last_reading.get('Intake Press psi', 3140)
#     press_status = "🟢 Normal" if 680 <= press_diff <= 720 else "🟠 Warning" if 650 <= press_diff <= 750 else "🔴 Critical"
#     st.metric("Pressure Differential (psi)", 
#               f"{press_diff:.0f}",
#               delta=press_status)
